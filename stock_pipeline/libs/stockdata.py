...
class StockDataClient:
    ...
    def getgeneraldata(self, endpoint, ticker=None, date_from=None):
        DATASOURCE = 'polygon'
        ...
        match endpoint:
            case 'ticker':
                ...
                return r.json()
            case 'financials':
                r = requests.get(url=url + '?ticker=' + ticker + '&filing_date.gte=' + date_from + '&limit=10', headers=header)
                return r.json()
            case 'div':
                r = requests.get(url=url + '?ticker=' + ticker + '&ex_dividend_date.gte=' + date_from + '&limit=10', headers=header)
                return r.json()
